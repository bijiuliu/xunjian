...
- 登录页使用内嵌透明 SVG，标准显示尺寸为 56px，不添加蓝色圆角底。图标与“夜班巡检”标题同时出现时使用 `aria-hidden`，避免辅助技术重复朗读。
...
首页同步状态使用紧凑的单行弱提示，不新增重复入口或常驻大卡片。文案必须准确区分“同步中”“离线 · N 项待同步”“同步失败 · N 项待同步”“已同步”和“已同步 · HH:mm”；待同步数量大于零时不能显示成已完成。最近同步时间只表示最近一次成功完成的账号同步，不代表尚在防抖等待的草稿已经提交。
