...
1. 在 Supabase 新建项目，按文件名顺序执行 `supabase/migrations/` 下的迁移。第一份创建巡检同步表，第二份创建用户偏好表、私有头像桶及 RLS 策略，`20260830040000_session_revocation_realtime.sql` 为 `user_preferences` 增加跨设备会话撤销字段并启用 Realtime。
